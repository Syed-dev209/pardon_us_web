...
class FilesUpload{
 ...
 Future<bool> uploadDetails(String quizTitle,String time,String date,String fileUrl,String classCode)async{
  ...
  {
    try {
      String fileUrl = await uploadFileWeb(assFile, classCode, 'assignmentFiles');
  ...
  Future<String> uploadFileWeb(PlatformFile _file,String classCode,String ref) async {
   ...
   html.File htmlFile=html.File(_file.bytes,_file.name);
    ...
    print('2');
    fb.UploadTaskSnapshot uploadTaskSnapshot = await storageReference.put(_file.bytes).future;
